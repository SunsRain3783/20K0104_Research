...
public class TimeTable extends AppCompatActivity implements View.OnClickListener{
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        gestureDetector = new GestureDetector(this, new TimeTable.SwipeGestureDetector());
    }
    ...
    public void onClick(View view){  //ボタンをクリックしたときの挙動
        if(view.getId() == R.id.mapIcon) {  //switch使うとエラー(if elseを使う)
            ...
            Intent intent = new Intent(getApplication(),ClassDetail.class);
            startActivity(intent);
        } else if(view.getId() == R.id.select2){
            Intent intent = new Intent(getApplication(),TimeTable2.class);
            startActivity(intent);
        }
    }
}